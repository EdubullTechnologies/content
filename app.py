...
from google import genai  # Updated to newer SDK
from google.genai import types  # Added for proper file handling
...
# Initialize Google Gemini Client with newer SDK
try:
    client = genai.Client(api_key=GOOGLE_API_KEY)
    MODEL_NAME = "gemini-2.5-pro-preview-05-06"  # Using the correct model name
...
def analyze_legal_document(file_bytes, filename, document_type, analysis_type, jurisdiction="Delhi, India"):
    """Analyze legal document using Gemini API with newer SDK."""
    ...
    try:
        ...
        # Upload file using the newer SDK File API
        ...
        # Generate analysis using newer SDK
            ...
            if hasattr(response, 'candidates') and response.candidates:
                ...
                if hasattr(candidate, 'content') and candidate.content:
                    if hasattr(candidate.content, 'parts') and candidate.content.parts:
                        analysis_result = ''.join([part.text for part in candidate.content.parts if hasattr(part, 'text')])
                    else:
                        analysis_result = str(candidate.content)
                else:
                    raise Exception("No content found in response candidate")
            else:
        ...
        try:
            ...
            st.info(f"Temporary file '{uploaded_file.name}' deleted from Gemini.")
...
def generate_legal_chat_response(user_prompt, chat_history, uploaded_files, jurisdiction="Delhi, India"):
    """Generate legal consultation response using newer SDK."""
    try:
...
"""
        ...
        if uploaded_files:
            for uploaded_file in uploaded_files:
                try:
                    ...
                    # Upload to Gemini using newer SDK
        ...
        # Generate response using newer SDK
        response = client.models.generate_content(
            model=MODEL_NAME,
